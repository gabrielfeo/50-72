...
class RealEnvironment(
    ...
    override fun gitCommentChar(): Char {
        val (exitCode, stdout) = commandRunner.run("git config --get core.commentChar")
        return when (exitCode) {
            1 -> '#'
            0 -> checkIsValidCommentChar(stdout.content.trim())
            else -> errorGettingCommentChar(exitCode)
        }
    }

    private fun errorGettingCommentChar(exitCode: Int): Nothing {
        error("Failed to get git commentChar setting. Exit $exitCode")
    }

    private fun checkIsValidCommentChar(string: String): Char {
        check(string.length == 1) { INVALID_COMMENT_CHAR_ERROR }
        return string.first()
    }
}
